...
export default function SupplierOnboardingPage() {
  ...
  async function loadProfile() {
    try {
      ...
      if (data.userId) {
        loadDocuments(data.userId, token);
      }
    } catch (err) {
  ...
  async function loadDocuments(userId: string, token: string) {
    try {
      const res = await fetch(`/api/v1/documents?ownerType=SUPPLIER&ownerId=${userId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  async function handleDocumentUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      await loadDocuments(profile.userId, token!);
  ...
  async function handleSubmitVerification() {
    ...
    try {
      ...
      const res = await fetch("/api/v1/supplier/verification/submit", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="space-y-1 flex-1 w-full">
                    ...
                    <input
                      ...
                      placeholder="sales@apexchemicals.example.com"
                      className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none bg-white"
                    />
                  </div>
