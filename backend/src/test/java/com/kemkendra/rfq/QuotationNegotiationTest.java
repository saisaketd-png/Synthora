...
public class QuotationNegotiationTest {
    ...
    @Autowired
    private com.kemkendra.notification.NotificationRepository notificationRepository;
    ...
    public void testCounterOfferAndRevisionNotificationDelivery() throws Exception {
        ...
        List<com.kemkendra.notification.Notification> supplierNotifications = notificationRepository.findByRecipientIdOrderByCreatedAtDesc(supplierUser.getId());
        assertFalse(supplierNotifications.isEmpty(), "Supplier should receive a persistent counter offer notification");

        com.kemkendra.notification.Notification counterNotif = supplierNotifications.get(0);
        assertEquals(com.kemkendra.notification.NotificationType.COUNTER_OFFER_RECEIVED, counterNotif.getType());
        ...
        assertEquals(com.kemkendra.notification.NotificationEntityType.RFQ, counterNotif.getEntityType());

        // Verify Buyer does NOT receive supplier-side counter offer notification
        List<com.kemkendra.notification.Notification> buyerNotifications = notificationRepository.findByRecipientIdOrderByCreatedAtDesc(buyer.getId());
        boolean buyerReceivedCounterNotif = buyerNotifications.stream()
                .anyMatch(n -> n.getType() == com.kemkendra.notification.NotificationType.COUNTER_OFFER_RECEIVED);
