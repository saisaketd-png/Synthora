...
public class UserSecurityTest {
    ...
    @Autowired
    private com.kemkendra.order.PurchaseOrderRepository purchaseOrderRepository;

    @Autowired
    private com.kemkendra.rfq.quotation.QuotationRepository quotationRepository;

    @Autowired
    private com.kemkendra.rfq.RfqRepository rfqRepository;
    ...
    public void testAdminCanGetUserById() throws Exception {
        mockMvc.perform(get("/api/v1/users/" + buyerUser.getId())
                ...
                .andExpect(jsonPath("$.email").value("buyer@kemkendra.com"));
    }
    ...
    public void testAuthenticatedUsersCanAccessUsersMe() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/users/me")
                        .header("Authorization", "Bearer " + supplierToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.email").value("supplier@kemkendra.com"));

        // Admin accessing /users/me
        mockMvc.perform(get("/api/v1/users/me")
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.email").value("admin@kemkendra.com"));
    }
