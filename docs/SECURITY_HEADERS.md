# Synthora HTTP Security Headers & Content Security Policy (CSP)

---

## 1. Overview

This document details the HTTP security headers and Content Security Policy (CSP) implemented across the Synthora B2B chemical marketplace frontend (Next.js 16), backend REST API (Spring Boot 3.4), and reverse proxy (Nginx).
...
| **`Permissions-Policy`** | Next.js, Spring Boot, Nginx | `camera=(), microphone=(), geolocation=(), payment=()` | `camera=(), microphone=(), geolocation=(), payment=()` | Disables browser hardware and sensor capabilities not required by Synthora. |
...
Enforced in [`frontend/next.config.ts`](file:///d:/Saisaket/Synthora/frontend/next.config.ts):
...
Enforced in [`backend/src/main/java/com/synthora/config/SecurityConfig.java`](file:///d:/Saisaket/Synthora/backend/src/main/java/com/synthora/config/SecurityConfig.java):
